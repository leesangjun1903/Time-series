...
> - PCA는 데이터의 변동성(분산)이 가장 큰 방향을 찾는 기법입니다. 이때 고유값은 그 방향으로 데이터가 얼마나 퍼져 있는지를 나타내는 '길이'와 같습니다.
>   - 1순위 방향의 고유값은 100이고, 2순위 방향의 고유값은 10이라고 해보겠습니다. 두 방향의 차이가 아주 명확합니다. 새로운 데이터를 몇 개 추가하거나 빼도 1순위와 2순위가 뒤바뀔 일이 거의 없습니다.
>   - 1순위 고유값은 50.1이고, 2순위 고유값은 49.9라고 해보겠습니다. 두 방향의 중요도가 거의 비슷합니다. 이때는 데이터가 아주 조금만 바뀌어도 1순위와 2순위의 우열이 쉽게 뒤집힙니다. 즉, 주성분 방향(고유벡터)이 나침반 바늘처럼 심하게 흔들리게 됩니다.

> - '고유값 분리가 표본 크기 요구량에 영향을 준다'는 말은 바로 이 흔들림을 잡기 위한 대가를 뜻합니다.
>   - 고유값 사이의 간격이 좁을수록, 주성분 방향을 정확하게 찾아내기 위해 지수적으로 더 많은 표본(데이터)이 필요합니다.
>   - 데이터가 아주 많아야만 미세한 차이(예: 50.1과 49.9)를 통계적으로 확실하게 구분할 수 있기 때문입니다.
>   - 반대로 표본 크기가 충분하지 않으면, 우연한 오차(노이즈) 때문에 주성분이 엉뚱한 방향을 가리키는 오류가 발생합니다.

5. **CCA는 고차원에서 overfit 위험이 큽니다.** $R_{11}^{-1},R_{22}^{-1}$이 불안정하면 regularized CCA가 필요합니다.

> CCA는 두 데이터 집합의 상관관계를 극대화하는 축을 찾기 위해 각 데이터의 공분산 행렬(Covariance Matrix)인 $\(R_{11}\)$ 과 $\(R_{22}\)$ 의 역행렬( $\(R_{11}^{-1}, R_{22}^{-1}\)$ )을 계산해야 합니다.

> - 데이터의 차원(변수 개수)이 샘플 수보다 많아지면, 행렬의 행과 열 사이에 빈틈이 생깁니다.
> - 이로 인해 행렬의 행렬식(Determinant)이 0에 가까워져 역행렬을 구하기가 불가능하거나 대단히 불안정해집니다. 이를 수학적으로 Ill-conditioned(병태 성격) 상태라고 합니다.
> - 역행렬이 불안정하면 작은 노이즈(오차)에도 결과 값이 크게 튀어, 학습 데이터에만 억지로 맞춘 잘못된 상관관계가 만들어집니다.

> - Regularized CCA (규제화 CCA)
>   - 이 문제를 해결하기 위해 $\(R_{11}\)$ 과 $\(R_{22}\)$ 의 대각선 성분에 의도적으로 작은 값(패널티)을 더해 행렬을 안정적인 상태로 강제 전환해 줍니다. 릿지 회귀(Ridge Regression)와 같은 원리입니다.
>   - $\(R_{11}\rightarrow R_{11}+\lambda_{1}I\)$ , $\(R_{22}\rightarrow R_{22}+\lambda_{2}I\)$
>   - $\(I\)$ (단위행렬): 대각선만 1이고 나머지는 0인 행렬입니다.
>   - $\(\lambda_1, \lambda_2\)$ (규제 매개변수): 얼마나 규제를 줄지 결정하는 작은 양수입니다.

>   - 규제화를 하면 역행렬이 항상 안전하게 계산되고, 변수들끼리 서로 강하게 얽혀 있는 다중공선성 문제를 막아줍니다. 데이터의 노이즈에 모델이 휘둘리지 않아 과적합을 방지합니다. 
...
> PCA는 데이터의 평균(Mean), 분산(Variance), 그리고 주성분 방향(Principal Direction)을 계산합니다. Test 데이터가 합쳐지면 Train 데이터만의 순수한 분포가 달라집니다.
...
> PCA는 분산(Variance, 데이터가 널리 퍼진 정도)이 큰 방향이 중요하다고 믿고, 분산이 작은 방향은 '노이즈(잡음)' 취급하여 버립니다. 하지만 실제 정답(Target)을 예측하는 데 결정적인 힌트가 이 '분산이 작은 방향'에 숨어 있을 수 있습니다. PCR은 이 힌트를 시작부터 버리고 가지만, Ridge는 버리지 않고 끝까지 가져갑니다.

> PCR은 특정 주성분을 넣을지 말지(0 아니면 1) 결정합니다. Ridge는 모든 방향의 영향을 유지하되, 덜 중요한 방향의 힘(가중치)을 연속적으로 줄여줍니다.
...
> - 설명된 분산 기준 : 데이터가 가진 원래 정보(분산)를 최대한 덜 잃어버리면서 차원을 줄이는 것이 목적입니다. 누적 설명 분산 그래프를 그렸을 때, 완만해지는 지점(Elbow)을 찾습니다. 보통 전체 분산의 70% ~ 90% 이상을 설명하는 지점의 성분 수를 선택합니다.
> - 검증 성능 기준 : 주성분 분석(PCA)은 데이터의 '분산(크기)'이 큰 순서대로 성분을 뽑아냅니다. 하지만 분산이 크다고 해서 무조건 예측에 중요한 변수인 것은 아닙니다. 때로는 분산이 작은 성분이 정답을 맞히는 데 결정적인 힌트가 될 수도 있습니다. 성분 수(n_components)를 1개, 2개, 3개... 등으로 바꾸어가며 검증 데이터(Validation Set)의 예측 점수(예: accuracy, RMSE)를 확인합니다. 가장 점수가 높은 성분 수를 최종 선택합니다.
