...
> 주성분분석(PCA)은 고차원 데이터를 분산이 가장 큰 방향(주성분)으로 투영하여 차원을 줄입니다. 이때 상위 $\(m\)$ 개의 주성분만 남기고 나머지를 버린 뒤, 이를 다시 원래 공간으로 복원(Reconstruction)하면 원본 데이터와 가장 유사한 최적의 저차원 근사치 $(\(\^{X}^{c}\))$ 를 얻을 수 있습니다.
