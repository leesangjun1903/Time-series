# Recipe 2-7. Simple Exponential Smoothing (SES)

> 학습 수준: **대학/대학원 → 연구자 발전형**  
> 원문 위치: **PDF 76–78 / book pp.62–64**  
...
6. 본 학습자료는 Recipe 2-7 “Simple Exponential Smoothing (SES)”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
이고, 모든 미래 지평에 대한 점예측은

$$
\hat{Y}_{t+h\mid t}=\ell_t
$$

입니다. $\alpha$가 크면 최근 관측에 빠르게 반응하고, 작으면 더 강하게 평활합니다.

> **용어 설명 — level state**  
> 현재 시계열의 기저 수준을 나타내는 잠재 상태입니다. SES는 trend와 seasonality 없이 level만 갱신합니다.
...
- $\ell_t$: 시점 $t$까지 업데이트된 level 추정치입니다.
- $Y_t$: 현재 관측값, $\ell_{t-1}$: 직전 level 추정치입니다.
- $\alpha\in(0,1)$: smoothing parameter이며 클수록 최근 관측에 더 큰 가중치를 줍니다.
- $\hat{Y}_{t+h\mid t}$: 시점 $t$에서 만든 $h$단계 미래 예측입니다.
- 기본 SES에서는 모든 미래 horizon의 point forecast가 현재 level $\ell_t$로 동일합니다.
...
저자는 smoothing level 0.8의 SES를 적용하고 RMSE 3536.576을 보고하며 trend가 있는 데이터에 부적합할 수 있다고 설명합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

SES는 가장 단순한 state-space forecasting baseline 중 하나로, 복잡한 모델이 실제로 level persistence 이상을 학습했는지 확인하는 기준선입니다.
...
smoothing level 0.8을 validation 없이 고정합니다. SES는 trend/seasonality가 있는 Bitcoin에 구조적으로 불리하므로 모델 간 비교가 공정한 benchmark라고 보기 어렵습니다.
