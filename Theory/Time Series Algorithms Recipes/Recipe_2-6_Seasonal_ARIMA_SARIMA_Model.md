# Recipe 2-6. Seasonal ARIMA (SARIMA) Model

> 학습 수준: **대학/대학원 → 연구자 발전형**  
> 원문 위치: **PDF 74–76 / book pp.60–62**  
...
6. 본 학습자료는 Recipe 2-6 “Seasonal ARIMA (SARIMA) Model”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
계절 주기 $m$을 갖는 SARIMA $(p,d,q)(P,D,Q)_m$은
...
로 표현합니다. 소문자 $p,d,q$는 비계절 구조, 대문자 $P,D,Q$는 계절 구조, $m$은 계절 주기입니다.

> **용어 설명 — seasonal differencing**  
> $(1-B^m)Y_t=Y_t-Y_{t-m}$처럼 한 계절 전 값과 차분하여 반복 계절 수준을 제거하는 연산입니다.

$D$와 $d$를 과도하게 주면 과차분이 발생할 수 있습니다. 또한 월별 데이터에서 $m=12$가 흔하지만, 실제 물리·운영 주기가 12인지 확인해야 합니다.
...
- $p,d,q$: 비계절 AR, 차분, MA 차수이고 $P,D,Q$는 대응되는 계절 차수입니다.
- $m$: 계절 주기입니다.
- $B$: backshift operator, $(1-B)^d$: ordinary differencing, $(1-B^m)^D$: seasonal differencing입니다.
- $\phi(B),\theta(B)$: 비계절 AR/MA polynomial입니다.
- $\Phi(B^m),\Theta(B^m)$: 계절 lag $m,2m,\ldots$에서 작동하는 계절 AR/MA polynomial입니다.
- $Y_t$: 원 시계열, $\varepsilon_t$: innovation입니다.
...
저자는 SARIMA를 ARIMA의 계절 확장으로 소개하고 (1,2,0)(2,2,2,12)를 적합해 RMSE 1050.157을 보고합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

SARIMA는 주기가 안정적이고 반복되는 데이터에서 매우 강력한 low-variance 모델이며, 작은 데이터에서 우선 고려할 가치가 큽니다.
...
seasonal order 선택 근거가 체계적이지 않고, 이전 recipe의 test 기반 선택 결과를 이어받았다면 간접적인 selection bias가 남습니다. seasonal differencing D=2도 과차분 위험을 점검해야 합니다.
