...
6. 본 학습자료는 Recipe 1-3A “Exploring Types of Time Series Data — Univariate”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
단변량 시계열은 하나의 관측 변수 $Y_t$의 과거가 미래 예측의 핵심 정보가 되는 문제입니다. 가장 일반적인 예측식은

$$
\hat{Y}_{t+h\mid t}=f(Y_t,Y_{t-1},\ldots,Y_{t-L+1})
$$

로 쓸 수 있습니다. $h$는 예측 지평(horizon), $L$은 look-back 길이, $f$는 예측 함수입니다. 단변량이라는 말은 **입력에 다른 변수가 절대 들어갈 수 없다는 뜻**이 아니라, 분석의 중심 확률과정이 하나라는 뜻으로 이해하는 편이 정확합니다.

> **용어 설명 — forecast horizon**  
> 현재 시점에서 몇 단계 앞을 예측하는지 나타내는 값입니다. $h=1$은 one-step ahead, $h=24$는 데이터가 시간 단위라면 24시간 앞 예측이 됩니다.

정상성 연구에서는 평균과 자기공분산을 봅니다.
...
약한 정상성에서는 $\mu$가 시간에 따라 변하지 않고 $\gamma(k)$가 시점 $t$ 자체가 아니라 lag $k$에만 의존한다고 가정합니다. 이 조건은 ARMA 계열 이론의 핵심입니다.
...
- $\hat{Y}\_{t+h\mid t}$: 시점 $t$까지의 정보만 사용해 $h$단계 미래 $Y\_{t+h}$를 예측한 값입니다. `hat`은 추정/예측값을 뜻합니다.
- $h$: forecast horizon, 즉 몇 단계 앞을 예측하는지 나타냅니다.
- $L$: 입력으로 사용할 과거 관측 개수인 look-back length입니다.
- $f(\cdot)$: 과거 시계열을 미래 예측으로 변환하는 예측 함수입니다.
- $\mu$: 정상 과정에서 시간과 무관한 평균 수준입니다.
- $\gamma(k)$: lag $k$의 자기공분산입니다.
- $\text{Cov}(A,B)$: 두 확률변수 $A,B$가 함께 변하는 정도를 측정하는 공분산입니다.
- $k$: 현재와 과거 사이의 lag 간격입니다.
...
저자는 하나의 시간의존 변수만 분석하는 경우를 univariate time series로 소개하고 일별 최저기온 예제를 시각화합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

단변량 모델은 baseline과 구조 이해에 매우 중요하며, 복잡한 multivariate 모델의 성능 향상이 실제로 외생변수 때문인지 확인하는 기준선이 됩니다.
...
시각화만으로 정상성·계절성·구조변화를 확정할 수 없습니다. 또한 본문에 기간 설명이 코드 데이터와 어긋날 가능성이 있어 원자료 확인이 필요합니다.
