...
6. 본 학습자료는 Recipe 2-4 “Autoregressive Integrated Moving Average (ARIMA) Model”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
### 3.1 ARIMA($p,d,q$)의 정확한 정의

ARIMA는 $d$차 차분한 시계열에 ARMA($p,q$)를 적용합니다.
...
$$
\phi(B)Z_t=c+\theta(B)\varepsilon_t.
$$

$B$는 backshift operator, $d$는 **차분의 차수**, $p$는 AR 차수, $q$는 MA 차수입니다. 중요한 점은 `shift(4)`로 $Y_t-Y_{t-4}$를 만드는 것과 $d=4$의 네 번 연속 차분 $(1-B)^4Y_t$는 수학적으로 다르다는 사실입니다.

shift(4)를 이용한 차분 (계절성 차분, $\(D=1, s=4\)$ ) :

$$
Y_t-Y_{t-4}=(1-B^4)Y_t,
$$

- shift(4)를 이용한 계절성 차분(Seasonal Differencing)과 $\(d=4\)$인 연속 차분(Regular Differencing)은 수학적으로 완전히 다른 연산이며, 시계열에 미치는 효과도 다릅니다.
- 이 연산은 현재 시점과 4기간 전의 시점(예: 분기별 데이터에서 전년 동기)을 직접 빼는 것입니다. Backshift operator로는 $\((1-B^4)\)$ 로 표현됩니다.
- 주기가 4인 계절성(Seasonal pattern)을 제거하기 위해 사용합니다. 계절성 ARIMA 모델인 SARIMA $\((p,d,q)\times(P,D,Q)_4\)$ 에서 계절성 차분 차수 $\(D=1\)$ 에 해당합니다.

반면 $\(d=4\)$ (4차 연속 차분) : 

$$
(1-B)^4Y_t=Y_t-4Y_{t-1}+6Y_{t-2}-4Y_{t-3}+Y_{t-4}.
$$

- 시계열 데이터에 존재하는 강력한 비선형 트렌드(예: 3차 다항식 형태의 추세)를 제거하여 정상성(Stationarity)을 확보하기 위해 사용합니다.

> **용어 설명 — over-differencing**  
> 필요한 것보다 지나치게 큰 $d$를 사용해 신호를 불필요하게 차분하는 현상입니다. 예측 분산이 증가하고 MA 구조가 인위적으로 생길 수 있습니다.

ADF 검정 하나만으로 $d$를 확정하기보다 KPSS, 그래프, ACF, 정보기준, 예측 검증을 함께 사용하는 것이 좋습니다.

> 그 이유는 통계적 검정 자체의 한계와 데이터가 가진 고유한 노이즈 때문입니다.

> ADF (Augmented Dickey-Fuller): 귀무가설이 "단위근이 존재한다(비정상성)"입니다. p-value가 작아야 정상 시계열입니다.

> KPSS (Kwiatkowski-Phillips-Schmidt-Shin): 귀무가설이 "시계열이 정상적이다"입니다. p-value가 커야(확립된 유의수준보다 높아야) 정상 시계열입니다.

> 두 검정은 귀무가설 $(\(H_{0}\))$ 이 서로 반대이므로, 함께 사용하면 데이터의 상태를 명확히 진단할 수 있습니다.

> 정보 기준 (AIC, BIC) 및 예측 검증 (Validation) : 결국 차분의 목적은 '예측력이 좋은 모델'을 만드는 것입니다.
>   - 정보 기준 최소화: $\(d=0, 1, 2\)$ 인 각각의 모델을 피팅한 후, AIC(Akaike Information Criterion)나 BIC(Bayesian Information Criterion) 값을 비교합니다. 일반적으로 정보 기준 점수가 가장 낮은 모델이 적절한 차수입니다.
>   - Out-of-sample 예측 검증: 데이터를 훈련(Train)과 검증(Validation) 세트로 나눕니다. $\(d=1\)$ 모형과 $\(d=2\)$ 모형의 검증 세트 오차(RMSE, MAE 등)를 비교하여, 실제 미래를 더 잘 맞추는 차수를 최종 선택합니다.

### 3.2 차분 연산의 정합성

책의 `shift(periods=4)`는 계절 lag 4 차분에 가깝고 ARIMA의 `d=4`는 네 번의 ordinary differencing을 뜻합니다. 두 설정을 동일한 의미로 연결하면 안 됩니다. 연구에서는 `d`와 seasonal period를 분리하고, 계절 차분이 필요하면 SARIMA의 $D,m$으로 명시하는 편이 낫습니다.

### 수식 기호 상세 해설

- $p,d,q$: ARIMA의 AR 차수, ordinary differencing 차수, MA 차수입니다.
- $B$: backshift operator이며 $B^kY_t=Y_{t-k}$입니다.
- $(1-B)^d$: 1차 차분 연산을 $d$번 반복한다는 뜻입니다.
- $Z_t$: $d$차 차분 후 만들어진 시계열입니다.
- $\phi(B),\theta(B)$: 각각 AR와 MA lag polynomial, $c$: 절편, $\varepsilon_t$: innovation입니다.
- $1-B^4$: 4시점 간격의 한 번의 lag-4 차분이고, $(1-B)^4$는 1차 차분을 네 번 반복한 연산입니다. 둘은 동일하지 않습니다.
...
저자는 차분 후 ADF/ACF/PACF를 사용하고 ARIMA(10,4,1)을 적합하여 RMSE 2895.31을 보고합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

ARIMA의 핵심은 차분으로 stochastic trend를 제거하는 것이지 높은 d를 주는 것이 아닙니다. 차분 차수 선택이 가장 중요한 모델링 결정 중 하나입니다.
...
- 정상성 검정 활용: ADF(Augmented Dickey-Fuller) 검정이나 KPSS 검정을 통해 통계적 유의성을 먼저 확인하세요. 일반적으로 1차 차분만으로도 대부분의 확률적 추세는 제거됩니다.
- ACF/PACF 시각화 점검: 1차 차분 후 ACF 플롯에서 첫 번째 시차(Lag 1)의 상관계수가 -0.5에 가깝거나 음수로 급격히 떨어진다면 과도 차분 상태를 의심해야 합니다.
- 분산 추적: 차분을 진행할 수록 데이터의 분산이 감소하다가, 과도 차분이 일어나는 시점부터 분산이 다시 증가합니다. 분산이 가장 작아지는 시점의 차수를 선택하는 것이 기본 원칙입니다.

## 6. 통계적으로 취약한 부분과 비교 불가능한 수치

가장 중요한 문제는 `Y_t-Y_{t-4}`를 만든 뒤 ARIMA의 `d=4`를 사용하는 논리입니다. seasonal lag-4 difference와 4차 ordinary difference는 다른 연산이므로 수학적 일관성을 재검증해야 합니다.
