# Recipe 2-8. Holt-Winters Model

> 학습 수준: **대학/대학원 → 연구자 발전형**  
> 원문 위치: **PDF 78–80 / book pp.64–66**  
...
6. 본 학습자료는 Recipe 2-8 “Holt-Winters Model”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
### 3.1 Holt와 Holt-Winters의 상태공간적 직관
...
계절성을 더한 가법 Holt-Winters는 계절 상태 $s_t$를 추가합니다.

$$
\hat{Y}_{t+h\mid t}=\ell_t+h b_t+s_{t-m+h_m}.
$$

> **용어 설명 — ETS**  
> Error, Trend, Seasonal의 조합으로 지수평활 모델을 상태공간 형태로 체계화한 계열입니다. 단순한 '평활 기법'보다 확률모형 관점에서 이해하면 예측구간과 likelihood를 자연스럽게 다룰 수 있습니다.
...
- $\ell_t$: level, $b_t$: trend/slope, $s_t$: seasonal state입니다.
- $\alpha$: level smoothing coefficient, $\beta$: trend smoothing coefficient입니다.
- $Y_t$: 현재 관측값, $h$: forecast horizon입니다.
- $m$: 계절 주기이며 $s_{t-m+h_m}$는 예측 대상 horizon에 대응되는 계절 상태입니다.
- $h_m$은 구현에 따라 horizon을 계절 위치로 환산하는 인덱스 표기입니다. 정확한 인덱싱은 사용 라이브러리 정의를 확인해야 합니다.
...
저자는 additive trend를 가진 ExponentialSmoothing을 적용해 RMSE 2024.683을 보고합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

Holt-Winters는 trend/seasonality를 명시적으로 분해하면서도 계산이 가벼워 운영 baseline으로 가치가 높습니다.
...
additive trend만 사용하며 seasonality 여부를 반영하지 않습니다. 단일 split과 단일 dataset 결과이므로 일반화 결론은 제한적입니다.
