...
6. 본 학습자료는 Recipe 2-2 “Autoregressive (AR) Model”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
AR($p$)는 현재가 자신의 $p$개 과거값의 선형 결합으로 생성된다고 가정합니다.
...
$c$는 절편, $\phi_i$는 lag $i$의 자기회귀계수, $\varepsilon_t$는 innovation입니다. AR(1)의 경우 $|\phi_1| < 1$이면 약한 정상성을 가지며
...
> **용어 설명 — innovation**  
> 과거 정보로 설명되지 않는 새로운 충격입니다. 좋은 시계열 모델이라면 innovation은 시간적으로 거의 상관이 없어야 합니다.

> 자기회귀계수는 시계열 데이터에서 과거의 값이 현재의 값에 미치는 영향력을 나타내는 숫자입니다.

PACF는 다른 중간 lag의 효과를 통제한 뒤 특정 lag와의 직접 선형 연관을 봅니다. 전통적으로 AR 차수 후보를 정하는 데 쓰지만, 현대 연구에서는 AICc/BIC 및 rolling-origin 검증과 함께 사용하는 것이 안전합니다.

- PACF(편자기상관계수, Partial Autocorrelation Function)는 시계열 데이터에서 두 시점 간의 순수한 상관관계를 측정하는 지표입니다. $\(k\))$ 에서의 상관관계를 보되, 그 사이에 있는 다른 시차들의 영향은 모두 제거(통제)한 상관계수입니다.
- 데이터가 순수한 $\(AR(p)\)$ 모형을 따를 때, PACF는 시차 $\(p\)$ 이후에 정확히 0으로 절단(Cut-off)되므로 차수 결정을 위한 직관적인 가이드라인을 제공합니다.
- AICc / BIC (정보 기준) : PACF의 모호한 시각적 판단을 넘어서, 모델의 적합도(Likelihood)와 파라미터 수에 대한 페널티를 계산하여 객관적인 숫자로 모델 후보군을 비교합니다.
- 시계열 데이터의 시간적 의존성(Data Leakage 방지)을 유지하면서 점진적으로 학습 데이터 창을 밀어내며 평가하는 Rolling-Origin(또는 Time Series Split) 방식을 통해서만, 모델의 '미래 데이터(Out-of-sample)'에 대한 실제 예측력을 안전하게 보장할 수 있습니다.

### 수식 기호 상세 해설

- $Y_t$: 현재값, $Y_{t-i}$: $i$시점 전의 과거값입니다.
- $c$: 절편, $p$: AR 차수, $\phi_i$: lag $i$의 자기회귀계수입니다.
- $\varepsilon_t$: 과거 정보로 설명되지 않는 innovation입니다.
- $\sigma^2$: innovation 분산입니다. $\varepsilon_t\sim(0,\sigma^2)$는 평균 0, 분산 $\sigma^2$인 충격을 가정한다는 간략 표기입니다.
- AR(1) 평균식의 $\phi_1$은 1시점 전 값의 지속성을 나타내며 $|\phi_1|<1$일 때 정상해의 평균 $c/(1-\phi_1)$이 존재합니다.
...
저자는 독일 전력 소비 데이터에서 ADF로 정상성을 확인하고 PACF를 통해 lag 8을 선택한 후 마지막 100개 관측을 test로 둔 AutoReg 모델을 시각화합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

AR는 해석 가능한 동적 회귀이며, 작은 표본에서 deep model보다 분산이 낮을 수 있습니다.
...
PACF의 시각적 cutoff만으로 lag 8을 확정하면 선택 불확실성이 반영되지 않습니다. residual diagnostics와 RMSE/MAE가 제시되지 않아 다른 모델과 정량 비교가 어렵습니다.
