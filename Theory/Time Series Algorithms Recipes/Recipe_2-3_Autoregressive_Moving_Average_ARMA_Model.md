...
> 현재의 시계열을 과거 관측치들의 선형 결합과 현재의 오차항으로 표현할 수 있다는 성질입니다. MA 부분의 계수가 특정 조건(MA 특성방정식의 근의 절대값이 1보다 커야 함)을 만족해야 합니다.

> - 무한 차수의 $\(AR(\infty)\)$ 형태 : $\(\epsilon_{t}=Y_{t}+\theta Y_{t-1}+\theta ^{2}Y_{t-2}+\theta ^{3}Y_{t-3}+\dots =\sum_{j=0}^{\infty }\theta ^{j}Y_{t-j}\)$

> - 만약 $\(\vert{}\theta\vert{} > 1\)$ 이라면 (예: $\(\theta = 2\)$ ):계수가 $\(2, 4, 8, 16, \dots\)$ 순으로 과거로 갈수록 기하급수적으로 커집니다. 100일 전 데이터가 오늘 데이터보다 현재 오차에 억만 배 더 큰 영향을 주게 되므로, 식이 발산하여 현실적인 계산이 불가능해집니다.
>   - '특성방정식의 근' : 시계열 분석에서는 식을 시차 연산자(Lag Operator, $\(B\)$ )를 사용해 표현합니다. 
>   - $(\(B Y_t = Y_{t-1}\))\(Y_{t}=(1-\theta B)\epsilon _{t}\)$ .
>   - 이때 $\(MA\)$ 특성방정식은 오차항 앞의 다항식을 $\(z\)$ 에 대한 방정식으로 만든 것입니다. : $\(1-\theta z=0\)$ .
>   - 이 방정식의 근 $(\(z\))$ 을 구하면 다음과 같습니다. : $\(z=\frac{1}{\theta }\)$

>  "과거의 계수가 수렴해야 한다( $\(\vert{}\theta\vert{} < 1\)$ )"는 조건이 복소수 공간이나 고차원 $\(MA(p)\)$ 모델로 확장되면서 "특성방정식의 근의 절대값이 1보다 커야 한다( $\(\vert{}z\vert{} > 1\)$ )"는 말과 완전히 같은 명제가 되는 것입니다.
