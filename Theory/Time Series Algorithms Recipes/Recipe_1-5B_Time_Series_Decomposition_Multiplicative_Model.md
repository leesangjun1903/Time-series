...
6. 본 학습자료는 Recipe 1-5B “Time Series Decomposition — Multiplicative Model”의 원문을 출발점으로 하되, 대학원 수준의 수학적 정의와 2020년 이후 연구를 연결합니다.
...
로 표현합니다. 수준 $T_t$가 커질수록 계절 진폭도 함께 증가하는 시계열에 적합할 수 있습니다. 모든 값이 양수라면 로그를 취해

$$
\log Y_t=\log T_t+\log S_t+\log R_t
$$

로 바꿀 수 있으므로, 승법 구조를 가법 구조로 분석할 수 있습니다.

> **용어 설명 — variance stabilization**  
> 값의 수준이 커질수록 분산도 커지는 현상을 로그·Box-Cox 변환 등으로 완화하는 것을 뜻합니다.

책의 '승법이면 비선형이고 주기 자체가 변한다'는 식의 설명은 엄밀하지 않습니다. 승법 분해의 핵심은 **성분이 곱으로 결합되고 계절 진폭이 수준과 비례할 수 있다는 점**이며, 주기(frequency)가 반드시 변해야 하는 것은 아닙니다.


### 3.2 승법 모델에 대한 엄밀한 수정

승법 분해는 “frequency가 변하는 모델”이 아닙니다. 보통 계절 주기 $m$은 고정되어 있고, 계절 **진폭**이 수준에 비례하는 현상을 모델링합니다. 실제 frequency가 시간에 따라 변한다면 time-varying seasonality, wavelet, state-space frequency model 같은 다른 도구가 필요합니다.

- 시계열 데이터의 주요 특징인 주기성(Seasonality)의 빈도(Frequency)나 패턴이 시간에 따라 변할 때(Time-varying), 전통적인 ARIMA나 고정된 Fourier 변환(FFT) 방식은 한계를 가집니다.
  - 가변 계절성 모델 (Time-varying Seasonality) : 주로 부가적(Additive) 또는 승법적(Multiplicative) 계절 성분에 시간에 따라 변하는 가중치를 부여합니다.
  - 웨이블릿 변환 (Wavelet Analysis) : 전체 데이터를 고정된 창으로 나누는 STFT(단시간 푸리에 변환)와 달리, 창의 크기를 유연하게 조절(고주파는 좁은 창, 저주파는 넓은 창)하여 해상도를 최적화합니다.
  - 상태 공간 모델 (State-Space Frequency Model) : 시스템의 내부 '상태(State)'가 매 시간단계마다 확률적으로 변한다고 가정하고, 관측 데이터로부터 이를 역추적합니다. 빈도 자체의 변화를 상태 방정식으로 정밀하게 제어할 수 있습니다. 칼만 필터(Kalman Filter)나 입자 필터(Particle Filter)를 기반으로 작동합니다. 시간에 따라 변하는 고유진동수(Time-varying frequency)를 상태 변수로 지정하여 실시간으로 추정합니다.

### 수식 기호 상세 해설

- $Y_t$: 관측값, $T_t$: trend, $S_t$: 계절 배율, $R_t$: 잔차 배율입니다.
- $\times$는 성분들이 원자료 수준에 비례하여 곱셈적으로 작용한다고 가정한다는 뜻입니다.
- $\log$는 자연로그를 의미하며 곱셈을 덧셈으로 바꾸어 해석하기 쉽게 만듭니다.
- 로그 변환식은 보통 $Y_t,T_t,S_t,R_t > 0$인 양수 영역에서 직접 사용할 수 있습니다.
...
저자는 Air Passengers를 승법 모델로 분해하고 seasonal component를 추출합니다.

이 문단은 **저자의 보고를 요약한 것**이며, 아래의 비판적 해석과 구분해야 합니다.

## 5. 비판적 해석 및 연구자 관점

로그 변환을 통해 승법 구조를 가법 구조로 바꾸는 것이 핵심 연결고리입니다.
...
승법 모델에서 계절 frequency 자체가 증가/감소해야 한다는 설명은 엄밀하지 않습니다. 0 또는 음수 데이터에는 승법 분해를 그대로 적용할 수 없습니다.
