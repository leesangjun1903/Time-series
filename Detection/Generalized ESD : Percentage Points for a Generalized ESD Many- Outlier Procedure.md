...
### 1. 핵심 개념: "반복적인 껍질 벗기기"
Generalized ESD는 단 하나의 이상치만 찾는 Grubbs 검정을 확장한 기법입니다.  
데이터셋에서 가장 튀는 놈을 하나씩 "후보"로 지목하고, 그놈을 제거했을 때 남은 데이터의 상태를 다시 확인하는 과정을 반복합니다.

### 2. 작동 메커니즘 (Step-by-Step)
#### ① 통계적 가설 설정
- 귀무가설( $H_{0}$ ): 데이터셋에 이상치가 존재하지 않는다.
- 대립가설( $H_{1}$ ): 최대 $k$ 개의 이상치가 존재한다. 

#### ② 편차 극대화 (Finding the Candidate)
매 단계마다 현재 데이터의 평균( $\bar{x}$ )과 표준편차( $s$ )를 구합니다.  
그 후, 평균에서 가장 멀리 떨어진 값을 찾습니다.

$R_{i} = \frac{max \left|\right. x_{i} - \bar{x} \left|\right.}{s}$

이 $R_{i}$ 가 바로 Grubbs 통계량이며, 현재 데이터 내에서 가장 "범죄 의심이 가는" 후보의 점수입니다.

##### 이 값은 z score 표준화 공식 아닌가?
통계학적으로 이를 'Grubbs 통계량' 또는 'Studentized Deviate'라고 따로 부르는 데에는 그 기능과 목적에 따른 전문적인 이유가 있습니다.

- $z$ -score: 특정 관측치가 평균으로부터 몇 표준편차만큼 떨어져 있는지 나타내는 '상대적 위치'를 측정합니다.
- Grubbs 통계량 ( $R_{i}$ ): 데이터셋 전체에서 '가장 극단적인(Extreme) 값' 하나만을 골라내어, 이 값이 통계적으로 "군계일학(이상치)"인지 심사하기 위한 '검정 통계량'으로 사용됩니다.

일반적인 z -score는 우리가 흔히 아는 1.96 이나 2.58 같은 고정된 표준정규분포 임계값을 사용합니다.  
하지만 Grubbs 검정(및 ESD)에서는 다음과 같은 이유로 훨씬 엄격한 기준을 적용합니다.

- 표본 크기의 반영: 데이터가 10개일 때 3표준편차만큼 떨어진 것과, 100만 개일 때 3표준편차 떨어진 것은 발생 확률이 완전히 다릅니다. Grubbs/ESD의 임계값( $\lambda_{i}$ )은 샘플 사이즈( $n$ )에 따라 매번 변합니다.
- 최댓값의 분포: 여러 데이터 중 "가장 큰 값"을 뽑았기 때문에, 이 값은 단순히 z -분포를 따르지 않고 Student's t-분포를 변형한 특수한 분포를 따릅니다.

코드( 0212_코드수정.md 참고 )에서 $R_{i}$ 를 구하는 과정을 보면:
- 가장 의심스러운 놈( z -score가 가장 높은 놈)을 찾는다.
- 그놈이 당시 데이터 상황( $n_i$ )에 맞게 설계된 엄격한 기준( $\lambda_{i}$ )을 통과하는지 본다.
- 통과하면 제거하고, 남은 사람들끼리 다시 z -score를 계산하고 최댓값을 찾는다.

즉, $R_{i}$ 는 매 라운드마다 선출되는 "가장 강력한 이상치 후보의 점수"이며, 이를 검증하는 $\lambda_{i}$ 는 그 라운드의 "통과 커트라인" 역할을 수행하는 것입니다.

#### ③ 엄격한 심사 기준 적용 (Critical Value Calculation)
데이터 개수( $n$ )가 줄어들수록 표준편차와 평균의 신뢰도가 변하므로, 임계값( $\lambda_{i}$ )을 매번 새로 계산합니다.  
이때 t-분포(Student's t-distribution)를 사용하는데, 이는 데이터가 적을 때의 불확실성을 반영하기 위함입니다.  
또한 본페로니 교정(Bonferroni Correction) 원리가 포함되어, 여러 번 검정하더라도 1종 오류(정상 데이터를 이상치로 오판)가 커지지 않도록 기준을 엄격하게 유지합니다.

#### ④ 순차적 제거와 판정
- 비교: $R_{i} > \lambda_{i}$ 이면 해당 후보는 통계적으로 유의미한 이상치로 간주될 가능성이 높습니다.
- 제거: 판정 여부와 상관없이 일단 해당 값을 데이터에서 제외하고, 남은 데이터로 다시 ①~③을 수행합니다. (최대 $k$ 번)
- 최종 결정: 기각 조건( $R_{i} > \lambda_{i}$ )을 만족하는 가장 큰 단계( $i$ )를 찾고, 그 단계까지 뽑혔던 모든 후보를 최종 이상치로 확정합니다.

### 3. 왜 이 방식이 전문적인가? (Grubbs와의 차별점)
#### 마스킹 효과(Masking Effect) 해결:
이상치가 2개 이상 모여 있으면 서로의 존재 때문에 표준편차가 커져서, 기존 Grubbs 검정으로는 둘 다 정상처럼 보일 때가 있습니다. ESD는 하나씩 제거하며 검정하므로 이 "가려짐 현상"을 깨뜨립니다.

#### 스왐핑 효과(Swamping Effect) 방지:
너무 강력한 이상치 하나 때문에 옆에 있는 정상 데이터까지 이상치로 오해받는 현상을 방지하기 위해, 단계별로 임계값을 정교하게 조정합니다.

### 4. 기술적 요약 (Technical Summary)
이 알고리즘은 "정규 분포 가정하에, 데이터의 오염(Contamination)을 순차적으로 제거하며 t-통계량의 신뢰 구간을 재설계하는 반복적 모수 검정법"이라고 정의할 수 있습니다.
